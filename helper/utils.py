...
def convert_json_to_dataframe(json_: dict) -> pd.DataFrame:
    ...
    for warehouse in warehouse_drivemode:
        ...
        warehouse_locations.append(
            f"({location}, {drive_time}, {drive_value}, {polygons})"
        )

    warehouse_polygon_df = pd.DataFrame(
        eval(",".join(warehouse_locations)),
        columns=["latitude", "longitude", "driving_time", "driving_mode", "polygon"],
    )

    warehouse_polygon_df["driving_mode"] = warehouse_polygon_df["driving_mode"].apply(
        lambda row: drive_maps[row]
    )
...
def points_inside_polygon(geo_json_poly, list_of_points=None):
    ...
    for idx, point in enumerate(list_of_points):
        ...
        list_of_points[idx] = (
            polygon.contains(point_obj),
            point,
        )  # check if polygon contains point
        point_geo = ({"type": "Point", "coordinates": point},)
        geometries.append(point_geo)
    geometries.append(geo_json_poly)
    return list_of_points


def generate_features_per_store(
    weight_parameters_in_polygon: pd.DataFrame,
) -> pd.DataFrame:
    """
    ...
    """
    weight_parameters_in_polygon["lat_long"] = weight_parameters_in_polygon[
        ["lat", "long"]
    ].apply(lambda row: (row.lat, row.long), axis=1)

    weighted_collection_in_polygon = (
        weight_parameters_in_polygon.groupby("pred_code")["lat_long"]
        .apply(list)
        .to_frame()
        .join(
            weight_parameters_in_polygon.groupby("pred_code")["num_customers"]
            .apply(list)
            .to_frame()
        )
        .join(
            weight_parameters_in_polygon.groupby("pred_code")["num_orders"]
            .apply(list)
            .to_frame()
        )
        .join(
            weight_parameters_in_polygon.groupby("pred_code")["num_items"]
            .apply(list)
            .to_frame()
        )
        .join(
            weight_parameters_in_polygon.groupby("pred_code")["gmv"]
            .apply(list)
            .to_frame()
        )
    )
...
def update_store_locations(
    lat: float, long: float, weight_parameters_in_polygon: pd.DataFrame
) -> pd.DataFrame:
    """
    Returns weight parameters for every location.
...
def get_covered_locations_per_store(
    covered_polygons_df: pd.DataFrame, queried_order_info: pd.DataFrame, pred_code: int
) -> pd.DataFrame:
    """
    Returns weight parameters for every location.
    ...
    weight_parameters_in_polygon = covered_polygons_df.join(
        queried_order_info.set_index(["lat", "long"])
    ).reset_index()
...
def get_covered_locations(row: pd.DataFrame, list_of_locations: list) -> pd.DataFrame:
    ...
    covered_polygons_df = pd.DataFrame(
        covered_polygons_df[1].to_list(), columns=["long", "lat"]
    )
...
def get_weighted_features_per_store(
    weight_parameters_in_polygon: pd.DataFrame, row: pd.DataFrame
) -> pd.DataFrame:
    """
    Returns weight parameters for every location.
    ...
    weight_parameters_in_polygon = generate_features_per_store(
        weight_parameters_in_polygon
    )
    weight_parameters_in_polygon = update_store_locations(
        lat, long, weight_parameters_in_polygon
    )
...
def get_location_overlapping_per_store(
    locations_matching_constraints: pd.DataFrame,
) -> pd.DataFrame:
    """
    Returns overlapped %  for every location.
    ...
    overlapped_df = (
        locations_matching_constraints.groupby(["lat", "long"])["pred_code"]
        .apply(list)
        .reset_index()["pred_code"]
        .to_frame()
    )
    overlapped_df["length"] = overlapped_df["pred_code"].apply(len)

    count_overlap = (
        locations_matching_constraints.groupby("pred_code")["pred_code"]
        .count()
        .to_frame()
    )
    ...
    count_overlap["overlap coverage %"] = (
        count_overlap["overlap_count"] / count_overlap["pred_code"]
    ) * 100
...
def get_latlong(locs: Tuple) -> Union[list, list]:
    """
    Returns splitted list of lat,long from locations.
...
def generate_feature_metrics(covered_per_actual_store: pd.DataFrame) -> pd.DataFrame:
    ...
    covered_per_actual_store["number_of_locations_covered"] = covered_per_actual_store[
        feature_columns[0]
    ].apply(lambda locs: len(locs))

    for feature_column in feature_columns[1:]:
        covered_per_actual_store[
            f"total_{feature_column}_coverage"
        ] = covered_per_actual_store[feature_column].apply(
...
def make_coverage_data(covered_per_actual_store):
    ...
    covered_per_actual_store["filtered_loc"] = get_filtered_locations(
        covered_per_actual_store
    )


    covered_per_actual_store["latlong_list"] = covered_per_actual_store[
        "filtered_loc"
    ].apply(lambda rows: split_list_into_features(rows)[0])
    covered_per_actual_store["customers_list"] = covered_per_actual_store[
        "filtered_loc"
    ].apply(lambda rows: split_list_into_features(rows)[1])
    covered_per_actual_store["orders_list"] = covered_per_actual_store[
        "filtered_loc"
    ].apply(lambda rows: split_list_into_features(rows)[2])
    ...
    covered_per_actual_store["latlong_list"] = covered_per_actual_store[
        "latlong_list"
    ].apply(lambda locs: get_latlong(locs))
...
def get_filtered_locations(covered_per_actual_store: pd.DataFrame) -> list:
    """
    Returns Unique Locations covered by each warehouse.
    ...
    for num, row in covered_per_actual_store.iterrows():
        new_loc = [
            (loc, cust, orders)
            for loc, cust, orders in zip(
                row[f"lat_long"], row["num_customers"], row["num_orders"]
            )
...
def get_weighted_driving_time(source_for_kpi_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in (
        source_for_kpi_metrics[["pred_lat", "pred_long"]].drop_duplicates().iterrows()
    ):
        one_store = source_for_kpi_metrics[
            (source_for_kpi_metrics["pred_lat"] == row["pred_lat"])
        ...
        for driving_time in timeline:
            ...
            avg_driving_time[driving_time] = (
                selected_polygons["driving_time"] * selected_polygons["total_customers"]
            ).sum() / selected_polygons["total_customers"].sum()
...
def calculate_avg_driving_time_at_store_level(
...
def get_customer_overlap_per_store(constrained_locations_unnest: pd.DataFrame) -> pd.DataFrame:
    """
    Returns overlapped %  for every location.

    Parameters:
        constrained_locations_unnest(DataFrame):  Locations matching constraints
    ...
    overlapped_df = (
        constrained_locations_unnest.groupby(["lat", "long"])[["pred_code", "num_customers"]]
            .agg(list)
            .reset_index()
    )
    ...
    overlapped_df["length"] = overlapped_df["pred_code"].apply(len)
    count_overlap = (
        constrained_locations_unnest.groupby("pred_code")["num_customers"].sum().to_frame()
    )

    no_overlap_df = overlapped_df[overlapped_df["length"] == 1][["pred_code", "num_customers"]]
    no_overlap_df = no_overlap_df.explode(['pred_code', 'num_customers'])

    count_overlap = count_overlap.join(
        no_overlap_df.groupby('pred_code')['num_customers'].agg(sum).to_frame().rename(
            columns={'num_customers': 'unique_customers'})
    )
    count_overlap['unique_customers'] = count_overlap['unique_customers'].fillna(0)

    count_overlap["overlap_customers"] = count_overlap["num_customers"] - count_overlap["unique_customers"]

    return count_overlap[['overlap_customers']]


def get_network_customer_overlap(locations_unnest_in_drive_time:pd.DataFrame) -> pd.DataFrame:
    """
    Returns overlapped %  for all location.

    Parameters:
        locations_unnest_in_drive_time(DataFrame):  Locations matching constraints

    Returns:
         overlapped_customers(int): total customers covered by more than one store
    """
    network_customers = locations_unnest_in_drive_time[['lat_long', 'num_customers']].drop_duplicates()[
        'num_customers'].sum()

    overlap_df = locations_unnest_in_drive_time.groupby(['lat', 'long'])[['num_customers', 'pred_code']].agg(
        list).reset_index()

    overlap_df['count'] = overlap_df['pred_code'].apply(len)

    non_overlap_customers = overlap_df[overlap_df['count'] == 1].explode(['num_customers'])['num_customers'].sum()

    overlapped_customers = network_customers - non_overlap_customers

    return overlapped_customers
